...
// Import icons
import createbranchIcon from '../../../../../assets/icons/branch-create.png';
import modifybranchIcon from '../../../../../assets/icons/branch-modify.png';
import mergebranchIcon from '../../../../../assets/icons/branch-merge.png';
import deactivatebranchIcon from '../../../../../assets/icons/branch-deactivate.png';
...
const getIconByType = (type: string): string => {
  const iconMap: Record<string, string> = {
    BRANCH_CREATION: createbranchIcon,
    BRANCH_MODIFY: modifybranchIcon,
    BRANCH_MERGE: mergebranchIcon,
    BRANCH_TRANSFER: modifybranchIcon,
    BRANCH_DEACTIVATE: deactivatebranchIcon,
  };
  return iconMap[type] || createbranchIcon;
};

// Label mapping based on operation type
const getLabelByType = (type: string): string => {
  const labelMap: Record<string, string> = {
    BRANCH_CREATION: 'Create',
    BRANCH_MODIFY: 'Modify',
    BRANCH_MERGE: 'Merge',
    BRANCH_TRANSFER: 'Transfer',
    BRANCH_DEACTIVATE: 'De-activate',
  };
...
const getRouteByType = (type: string): string => {
  const routeMap: Record<string, string> = {
    BRANCH_CREATION: '/re/sub-user-management/branch/list/create',
    BRANCH_MODIFY: '/re/sub-user-management/branch/list/modify',
    BRANCH_MERGE: '/re/sub-user-management/branch/list/merge',
    BRANCH_TRANSFER: '/re/sub-user-management/branch/list/transfer',
    BRANCH_DEACTIVATE: '/re/sub-user-management/branch/list/de-activate',
  };
  return routeMap[type] || '/re/sub-user-management/branch';
};
...
const BranchDashboard: React.FC = () => {
  ...
  // Fetch dashboard data from API and extract BRANCH operations
  useEffect(() => {
      ...
      try {
        ...
        if (response.data && response.data.data) {
          ...
          if (userManagementModule && userManagementModule.subModules) {
            // Find BRANCH subModule
            const branchSubModule = userManagementModule.subModules.find(
              (subModule) => subModule.type === 'BRANCH'
            );

            if (branchSubModule && branchSubModule.operations) {
              // Map operations to tasks
              const mappedTasks: TaskItem[] = branchSubModule.operations.map(
                (operation) => ({
            ...
            setError('Branch data not found in API response');
            setTasks([]);
          }
        } else {
