...
// Icon mapping based on subModule type
...
const getRouteByType = (type: string): string => {
  const routeMap: Record<string, string> = {
    USER: '/re/sub-user-management/user',
    REGION: '/re/sub-user-management/region',
    BRANCH: '/re/sub-user-management/branch',
  };
  return routeMap[type] || '/re/sub-user-management';
};
...
const SubUserDashboard: React.FC = () => {
  ...
  // Fetch dashboard data from API and extract USER_MANAGEMENT subModules
  useEffect(() => {
      ...
      try {
        ...
        if (response.data && response.data.data) {
          ...
          if (userManagementModule && userManagementModule.subModules) {
            ...
            const mappedTasks: TaskItem[] = userManagementModule.subModules.map(
              (subModule) => ({
                label: subModule.name,
                icon: getIconByType(subModule.type),
