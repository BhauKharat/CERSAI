...
import deactivateIcon from '../../../../../assets/icons/De-activate.png';
...
const getIconByType = (type: string): string => {
  const iconMap: Record<string, string> = {
    REGION_CREATION: createIcon,
    REGION_MODIFY: modifyIcon,
    REGION_MERGE: mergeIcon,
    REGION_DEACTIVATE: deactivateIcon,
  };
  return iconMap[type] || createIcon;
};

// Label mapping based on operation type
const getLabelByType = (type: string): string => {
  const labelMap: Record<string, string> = {
    REGION_CREATION: 'Create',
    REGION_MODIFY: 'Modify',
    REGION_MERGE: 'Merge',
    REGION_DEACTIVATE: 'De-activate',
  };
...
const getRouteByType = (type: string): string => {
  const routeMap: Record<string, string> = {
    REGION_CREATION: '/re/sub-user-management/region/list/create',
    REGION_MODIFY: '/re/sub-user-management/region/list/modify',
    REGION_MERGE: '/re/sub-user-management/region/list/merge',
    REGION_DEACTIVATE: '/re/sub-user-management/region/list/de-activate',
  };
  return routeMap[type] || '/re/sub-user-management/region';
};
...
const RegionDashboard: React.FC = () => {
  ...
  // Fetch dashboard data from API and extract REGION operations
  useEffect(() => {
      ...
      try {
        ...
        if (response.data && response.data.data) {
          ...
          if (userManagementModule && userManagementModule.subModules) {
            // Find REGION subModule
            const regionSubModule = userManagementModule.subModules.find(
              (subModule) => subModule.type === 'REGION'
            );

            if (regionSubModule && regionSubModule.operations) {
              // Map operations to tasks
              const mappedTasks: TaskItem[] = regionSubModule.operations.map(
                (operation) => ({
            ...
            setError('Region data not found in API response');
            setTasks([]);
          }
        } else {
