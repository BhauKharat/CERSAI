...
const UserSuspensionRequest: React.FC = () => {
  ...
  const [suspensionDetails, setSuspensionDetails] = useState({
    suspensionInitiatedBy: '',
    suspensionInitiatedOn: '',
    suspensionRemark: '',
  });
  ...
  useEffect(() => {
    if (id) {
      ...
      const populateFormFromWorkflowData = (workflowData: any) => {
        ...
        // Extract suspension remark from userDetails
        const suspensionRemark =
          userDetails.remarks || userDetails.reason || '';

        if (suspensionDetailsFromApi) {
          setSuspensionDetails({
            suspensionInitiatedBy: `${suspensionDetailsFromApi.suspendedByName} [${suspensionDetailsFromApi.suspendedBy}]`,
            suspensionInitiatedOn: formatDateOnly(
              suspensionDetailsFromApi.suspendedOn
            ),
            suspensionRemark: suspensionDetailsFromApi.suspensionRemark,
          });
        } else {
          setSuspensionDetails({
            suspensionInitiatedBy: initiatedByDisplay,
            suspensionInitiatedOn: formatDateOnly(
        ...
        if (userIdToFetch) {
          dispatch(fetchSubUserUserById(userIdToFetch))
            .then((action: any) => {
              ...
              console.log('Fetched user data for suspension:', userData);
      ...
      if (passedUserData) {
        ...
        const workflowPayload = {
          ...
          workflowType: 'RE_USER_SUSPENSION',
        };
    ...
    try {
      ...
      if (
        ...
        setModalState({
          ...
          type: 'reject',
          title: '',
          message: 'User Suspension Request Rejected',
          listBefore: [],
          listAfter: [],
        });
      } else {
  ...
  const handleModalClose = () => {
    setModalState((prev) => {
      ...
      if (prev.type === 'approve' || prev.type === 'reject') {
        ...
        setTimeout(() => {
          navigate('/re/sub-user-management/user/list/suspend');
        }, 100);
      }
  ...
  const crumbsData = [
    ...
    { label: 'Suspend', path: '/re/sub-user-management/user/list/suspend' },
    { label: 'Approval' },
  ];
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDayjs}>
            ...
            >
              <Grid container spacing={2}>
                <Grid size={{ xs: 12, sm: 12, md: 4 }}>
                  ...
                  >
                    {suspensionDetails.suspensionInitiatedBy || '-'}
                  </Typography>
                </Grid>
                <Grid size={{ xs: 12, sm: 12, md: 4 }}>
                  <Typography
                  ...
                  >
                    {suspensionDetails.suspensionInitiatedOn || '-'}
                  </Typography>
                </Grid>
                <Grid size={{ xs: 12, sm: 12, md: 4 }}>
                  <Typography
      ...
      <RejectConfirmationModal
        ...
        title="Are you sure you want to reject user suspension request?"
        remarkLabel="Remark"
