...
} from '../../../UserManagement/CreateModifyUser/slice/suspensionDetailsSlice';
...
const RevokeUserSuspensionRequest: React.FC = () => {
  ...
  // Get suspension details from Redux
  const { data: suspensionDetailsFromApi } = useSelector(
    (state: RootState) => state.suspensionDetails
  );
  ...
  const [revocationDetails, setRevocationDetails] = useState({
    revocationInitiatedBy: '',
    revocationInitiatedOn: '',
    revocationRemark: '',
  });
  ...
  // Fetch suspension details from API when component mounts
  useEffect(() => {
    if (id) {
  ...
  useEffect(() => {
    if (id) {
      ...
      const populateFormFromWorkflowData = (workflowData: any) => {
        ...
        const suspensionDetailsPayload =
          workflowData?.payload?.suspensionDetails || {};
        ...
        // Set revocation details (current revocation request)
        const revocationInitiatedBy =
          initiatorDetails.actionByUserName || metaData.initiatedBy || '';
        const revocationInitiatedByUserId =
          initiatorDetails.userId || metaData.initiatedBy || '';
        const revocationInitiatedByDisplay =
          revocationInitiatedBy && revocationInitiatedByUserId
            ? `${revocationInitiatedBy} [${revocationInitiatedByUserId}]`
            : revocationInitiatedBy || revocationInitiatedByUserId || '-';

        // Extract revocation remark from userDetails
        const revocationRemark =
          userDetails.remarks || userDetails.reason || '';

        setRevocationDetails({
          revocationInitiatedBy: revocationInitiatedByDisplay,
          revocationInitiatedOn: formatDateOnly(
            initiatorDetails.actionDateTime ||
              metaData.lastActionOn ||
              workflowData.created_at ||
              ''
          ),
          revocationRemark: revocationRemark,
        });

        // Set suspension details (original suspension)
        // Prioritize API data if available, otherwise use workflow data
        if (suspensionDetailsFromApi) {
          setSuspensionDetails({
            suspendedBy: `${suspensionDetailsFromApi.suspendedByName} [${suspensionDetailsFromApi.suspendedBy}]`,
            suspendedOn: formatDateOnly(suspensionDetailsFromApi.suspendedOn),
            suspensionRemark: suspensionDetailsFromApi.suspensionRemark,
          });
        } else {
          // Extract suspension details from workflow payload
          // Check suspensionDetails object first, then fall back to initiatorDetails
          const suspendedBy =
            suspensionDetailsPayload.suspendedBy ||
            suspensionDetailsPayload.initiatedBy ||
            suspensionDetailsPayload.actionByUserName ||
            initiatorDetails.actionByUserName ||
            '';
          const suspendedByUserId =
            suspensionDetailsPayload.suspendedByUserId ||
            suspensionDetailsPayload.initiatedByUserId ||
            suspensionDetailsPayload.userId ||
            initiatorDetails.userId ||
            '';
          const suspendedByDisplay =
            suspendedBy && suspendedByUserId
              ? `${suspendedBy} [${suspendedByUserId}]`
              : suspendedBy || suspendedByUserId || '-';

          const suspendedOn =
            suspensionDetailsPayload.suspendedOn ||
            suspensionDetailsPayload.initiatedOn ||
            suspensionDetailsPayload.actionDateTime ||
            initiatorDetails.actionDateTime ||
            '';

          const suspensionRemark =
            suspensionDetailsPayload.remarks ||
            suspensionDetailsPayload.reason ||
            initiatorDetails.remarks ||
            initiatorDetails.reason ||
            '';

          setSuspensionDetails({
            suspendedBy: suspendedByDisplay,
            suspendedOn: formatDateOnly(suspendedOn),
            suspensionRemark: suspensionRemark,
          });
        }
        ...
        if (userIdToFetch) {
          dispatch(fetchSubUserUserById(userIdToFetch))
            .then((action: any) => {
              ...
              console.log('Fetched user data for revoke suspension:', userData);
      ...
      if (passedUserData) {
        ...
        const workflowPayload = {
          ...
          workflowType: 'RE_USER_SUSPENSION_REVOKE',
        };
    ...
    try {
      ...
      if (
        ...
        setModalState({
          ...
          type: 'reject',
          title: '',
          message: 'Revocation of User Suspension Request Rejected',
          listBefore: [],
          listAfter: [],
        });
      } else {
  ...
  const handleModalClose = () => {
    setModalState((prev) => {
      ...
      if (prev.type === 'approve' || prev.type === 'reject') {
        ...
        setTimeout(() => {
          navigate('/re/sub-user-management/user/list/revoke-suspension');
        }, 100);
      }
  ...
  const crumbsData = [
    ...
    { label: 'User', path: '/re/sub-user-management/user' },
    {
      ...
      path: '/re/sub-user-management/user/list/revoke-suspension',
    },
    { label: 'Approval' },
  ];
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDayjs}>
            ...
            >
              <Grid container spacing={2}>
                <Grid size={{ xs: 12, sm: 12, md: 4 }}>
                  ...
                  >
                    {revocationDetails.revocationInitiatedBy || '-'}
                  </Typography>
                </Grid>
                <Grid size={{ xs: 12, sm: 12, md: 4 }}>
                  <Typography
                  ...
                  >
                    {revocationDetails.revocationInitiatedOn || '-'}
                  </Typography>
                </Grid>
                <Grid size={{ xs: 12, sm: 12, md: 4 }}>
                  <Typography
                  ...
                  >
                    {revocationDetails.revocationRemark || '-'}
                  </Typography>
                </Grid>
              </Grid>
            </Box>
      ...
      <RejectConfirmationModal
        ...
        title="Reject Revocation of User Suspension"
        remarkLabel="Remark"
...
export default RevokeUserSuspensionRequest;
