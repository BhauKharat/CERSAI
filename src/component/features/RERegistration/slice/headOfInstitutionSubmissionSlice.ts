...
import {
  HeadOfInstitutionSubmissionResponse,
  HeadOfInstitutionSubmissionState,
  HeadOfInstitutionMetadata,
} from '../types/headOfInstitutionSubmissionTypes';
import { FormField } from '../types/formTypes';

// Initial state
const initialState: HeadOfInstitutionSubmissionState = {
  loading: false,
  success: false,
  error: null,
  response: null,
...
// Async thunk to submit head of institution details
export const submitHeadOfInstitution = createAsyncThunk(
  'headOfInstitutionSubmission/submitHeadOfInstitution',
  async (
    {
      formValues,
      workflowId,
    ...
    try {
      ...
      const metadata: HeadOfInstitutionMetadata = {};
      ...
      console.log('📋 Final HOI metadata JSON:', metadataJson);

      // Validate metadata JSON before sending
      try {
        JSON.parse(metadataJson);
        console.log('✅ HOI metadata JSON is valid');
      } catch (error) {
        console.error('❌ Invalid HOI metadata JSON:', error);
      ...
      console.log('📊 Complete HOI FormData structure:', formDataEntries);
      ...
      const response = await Secured.post(
        API_ENDPOINTS.submit_head_of_institution,
        formData
      );

      console.log(
        '✅ Head of Institution submission successful:',
        response.data
      );
      return response.data as HeadOfInstitutionSubmissionResponse;
    } catch (error: unknown) {
      ...
      if (error && typeof error === 'object' && 'response' in error) {
        ...
        console.error('📋 HOI error details:', {
        ...
        if (
          ...
          console.error('🔍 HOI field validation errors:', errorData.errors);
          ...
          return rejectWithValue({
            ...
            message:
              ...
              'Head of Institution form validation failed',
        ...
        const errorMessage =
          ...
          axiosError.message ||
          'Failed to submit head of institution details';
...
// Head of Institution Submission slice
const headOfInstitutionSubmissionSlice = createSlice({
  name: 'headOfInstitutionSubmission',
  initialState,
  reducers: {
  ...
  extraReducers: (builder) => {
    builder
      .addCase(submitHeadOfInstitution.pending, (state) => {
        state.loading = true;
        state.success = false;
        state.error = null;
        state.response = null;
      })
      .addCase(submitHeadOfInstitution.fulfilled, (state, action) => {
        state.loading = false;
        state.success = true;
        state.error = null;
        state.response = action.payload;
      })
      .addCase(submitHeadOfInstitution.rejected, (state, action) => {
        state.loading = false;
        state.success = false;
...
export const {
  ...
  setSubmissionError,
} = headOfInstitutionSubmissionSlice.actions;

// Export reducer
export default headOfInstitutionSubmissionSlice.reducer;

// Root state interface for selectors
interface RootState {
  headOfInstitutionSubmission: HeadOfInstitutionSubmissionState;
}

// Selectors
export const selectHOISubmissionLoading = (state: RootState) =>
  state.headOfInstitutionSubmission.loading;
export const selectHOISubmissionSuccess = (state: RootState) =>
  state.headOfInstitutionSubmission.success;
export const selectHOISubmissionError = (state: RootState) =>
  state.headOfInstitutionSubmission.error;
export const selectHOISubmissionResponse = (state: RootState) =>
  state.headOfInstitutionSubmission.response;
