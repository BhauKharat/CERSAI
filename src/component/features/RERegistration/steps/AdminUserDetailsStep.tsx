...
const AdminUserDetailsStep: React.FC<AdminUserDetailsStepProps> = ({
  ...
  const reduxVerifiedSections = useSelector((state: RootState) =>
    selectAdminUserDetailsVerifiedSections(state)
  );
  const reduxSectionDataHashes = useSelector((state: RootState) =>
    selectAdminUserDetailsSectionDataHashes(state)
  );

  // Convert Redux array to Set for compatibility (memoized to prevent unnecessary re-renders)
    ...
    if (stepData && stepData.data && Object.keys(stepData.data).length > 0) {
      ...
      Object.entries(stepData.data).forEach(([fieldName, fieldValue]) => {
        if (
          ...
          // Mark CKYC field as auto-populated to show verify icon by default
          const lower = String(fieldName).toLowerCase();
          if (
            lower.includes('ckyc') &&
            (lower.includes('number') || lower.includes('no'))
          ) {
            dispatch(
              updateFormValue({
                fieldName: `${fieldName}_autoPopulated`,
                value: 'true',
              })
            );
          }
  ...
  }, [groupedFields, verifiedSections]);
        ...
        maskedMobile={
          currentValidatingSection
            ? (() => {
                ...
                console.log('🔍 OTP Modal - Mobile Number:', {
                  fieldName: `iauMobileNumber${currentValidatingSection.sectionName === 'adminone' ? '1' : '2'}`,
                  value,
                  allFormValues: Object.keys(formValues),
                });
