...
import { updateFieldValidation, updateFormValue } from '../slice/formSlice';
  ...
  const handleSave = useCallback(
    async (formData: Record<string, unknown>) => {
      ...
      if (flattenedFields && flattenedFields.length > 0) {
        flattenedFields.forEach((field: FormField) => {
      ...
      Object.entries(stepData.data).forEach(([fieldName, fieldValue]) => {
        if (
          ...
          if (fieldName === 'hoiCitizenship') {
            const hoiCkycNoFieldIndex = formFields.findIndex(
            ...
            if (hoiCkycNoFieldIndex !== -1) {
              ...
              updatedFields[hoiCkycNoFieldIndex] = {
                ...updatedFields[hoiCkycNoFieldIndex],
                validationRules: {
                  required: stringValue === 'Indian',
                },
              };
              dispatch(
                updateFieldValidation(updatedFields[hoiCkycNoFieldIndex])
              );
            }
          }
        }
      ...
      <FormErrorBoundary>
        <FieldErrorProvider fieldErrors={fieldErrors}>
          <DynamicForm
            ...
            prefilledDisablesValidate={true}
