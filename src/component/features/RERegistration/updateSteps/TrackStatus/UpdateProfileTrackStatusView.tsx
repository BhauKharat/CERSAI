...
} from '../slice/formPreviewTrackStatusSlice';
import {
  generateUpdatePdf,
...
const UpdateProfileTrackStatusView: React.FC<
...
> = () => {
  ...
  // Derive formData directly from workflowData to ensure it's always in sync
  const formData = useMemo(() => {
    const data = workflowData?.payload || {};
    return data;
  }, [workflowData]);
  ...
  // Helper to format submittedOn date from API (ISO string) to DD/MM/YYYY hh:mmAM
  ...
  // Note: formData is now derived directly from workflowData using useMemo above
  // No need for a separate useEffect to populate it
  ...
  const renderField = useCallback(
    (field: FormField) => {
      ...
      // Log field value lookup for debugging (sample fields only)
      ...
      // Hide the field unless the main pincode field has "other" or "others" value
      if (
  ...
  const getDisplayTitle = useCallback(
    (groupedFieldKey: string): string => {
      ...
      // Final fallback: Use the original label from API
      return section?.label || groupedFieldKey;
    },
    [groupedFields]
  );
  ...
  }, [groupedFields, renderFieldGroup, getDisplayTitle]);
    ...
    >
      ...
      >
        <Typography
          ...
          onClick={async () => {
            // Find the signed registration PDF in documents
            ...
            if (Array.isArray(documents)) {
              signedDoc = documents.find(
                (doc: any) =>
                  doc.fieldKey === 'registration_signed_application_pdf.pdf' ||
                  doc.fieldKey === 'registration_signed_application_pdf' ||
                  doc.type === 'registration_signed_application_pdf' ||
                  doc.fieldKey?.includes('registration_signed') ||
                  doc.type?.includes('registration_signed')
              );
            }
      ...
      >
        ...
        <Box sx={{ paddingLeft: '10px' }}>
          ...
          >
            {formatSubmittedOn(
              workflowData?.payload?.submittedAt || workflowData?.submittedOn
            )}
          </Typography>
        </Box>
      </Box>
      ...
      <Box sx={{ padding: 2 }}>
        <h1>Reporting Entity Details</h1>
      </Box>
      {/* Error Alert */}
...
export default UpdateProfileTrackStatusView;
