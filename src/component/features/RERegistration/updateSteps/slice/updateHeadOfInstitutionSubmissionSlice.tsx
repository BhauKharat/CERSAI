...
export interface HoiDetailsSubmissionResponse {
  status: string;
  message: string;
  data?: any;
}

export const submitUpdatedHoiDetails = createAsyncThunk(
  'hoiDetailsSubmission/submitHoiDetails',
  async (
    {
      formValues,
      userId,
    ...
    try {
      ...
      const response = await Secured.put(
        API_ENDPOINTS.submit_update_hoi_details,
        formData,
        {
          headers: {
      ...
      return response.data as HoiDetailsSubmissionResponse;
    } catch (error: unknown) {
      console.error('❌ HOI details submission failed:', error);
      ...
      const errorMessage =
        error instanceof Error ? error.message : 'Failed to submit HOI details';
