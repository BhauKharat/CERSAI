...
export const fetchStepDataUserProfile = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      // Use dataKey to extract the correct data (nodalOfficer or institutionalAdminUser)
      const stepData = rows.length > 0 ? rows[0].data?.[dataKey] || {} : {};
  ...
  async (
    {
      ...
      changedFieldsData?: Record<string, any>;
    },
    { rejectWithValue }
  ) => {
    try {
      ...
      // Add changedFields as JSON string if provided
      if (changedFields && changedFields.length > 0) {
        formData.append('changedFields', JSON.stringify(changedFields));
        console.log('📤 Submitting with changedFields:', changedFields);
      }

      // Add userId
      formData.append('userId', userId);

      // Add formType (dynamic based on user type)
      ...
      // Extract changedFields from API response if it exists, otherwise use submitted fields
      const responseChangedFields =
        response.data?.changedFields || changedFields;

      // Log the response with changedFields information
      console.log('✅ API Response received:', {
        ...response.data,
        changedFieldsSubmitted: changedFields,
        changedFieldsInResponse: responseChangedFields,
      });

      // Return response with changedFields included for tracking
      // If backend returns changedFields, use that; otherwise use the submitted fields
      return {
        ...response.data,
        changedFields: responseChangedFields, // Dynamically use API response or submitted fields
      };
    } catch (error: any) {
...
// Final Submit after eSign - calls the /submit endpoint to finalize the workflow
export const finalSubmitUserProfile = createAsyncThunk(
  ...
  async (
    ...
    try {
      console.log('📤 Final submit (finalize workflow) with:', params);
...
// Delete document
export const deleteDocumentUserProfile = createAsyncThunk(
...
const updateUserProfileSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(deleteDocumentUserProfile.pending, (state, action) => {
        // Mark document as being deleted
        const documentId = action.meta.arg;
        console.log(`🗑️ Deleting document: ${documentId}`);
      })
      .addCase(deleteDocumentUserProfile.fulfilled, (state, action) => {
        const documentId = action.meta.arg;
      ...
      .addCase(deleteDocumentUserProfile.rejected, (state, action) => {
        const documentId = action.meta.arg;
