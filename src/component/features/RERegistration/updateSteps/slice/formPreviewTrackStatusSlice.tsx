...
export const fetchWorkflowData = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      // Extract the workflow data from response
      ...
      // Current update-user-profile track-status response shape:
      // {
      //   workflowId: "...",
      //   workflowType: "RE_ENTITY_PROFILE_UPDATE",
      //   status: "IN_PROGRESS",
      //   currentStep: "RE_addressDetails",
      //   submittedOn: "...",
      //   submittedBy: "...",
      //   profileData: { hoi: {...}, addresses: {...}, nodalOfficer: {...}, entityDetails: {...} },
      //   documents: [ ... ],
      //   executedSteps: [ ... ]
      // }

      const workflowIdFromResponse = responseData.workflowId || workflowId;
      const workflowPayload = responseData.payload || {};
      ...
      // documents is an array in the current API; fall back to object handling if needed
      ...
      return {
        success: true,
        data: {
          ...responseData, // Include other workflow metadata first
          workflowId: workflowIdFromResponse,
          payload: flattenedPayload, // Flattened form data (overrides responseData.payload)
...
// Form Preview slice
const formPreviewTrackStatusSlice = createSlice({
  name: 'formPreview',
  initialState,
  reducers: {
...
export const { clearFormPreview, clearPreviewError } =
  formPreviewTrackStatusSlice.actions;
...
export default formPreviewTrackStatusSlice.reducer;
