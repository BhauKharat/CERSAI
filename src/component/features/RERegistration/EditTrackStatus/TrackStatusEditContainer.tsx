...
import { fetchWorkflowData } from '../slice/trackStatusFormSlice';
...
const DEFAULT_STEP_URLS = [
  'modify-entity-profile',
  'modify-address-details',
  'modify-head-of-institution',
  'modify-nodal-officer',
...
const getStepUrlFromFormType = (formType: string): string => {
  const urlMap: Record<string, string> = {
    RE_entity_profile: 'modify-entity-profile',
    RE_addressDetails: 'modify-address-details',
    RE_hoi: 'modify-head-of-institution',
    RE_nodal: 'modify-nodal-officer',
    RE_iau: 'modify-admin-user-details',
    RE_formPreview: 'modify-form-preview',
  };
  return urlMap[formType] || formType.toLowerCase().replace(/_/g, '-');
};

const TrackStatusEditContainer: React.FC = () => {
  ...
  const getCompletedStepsFromReinitialize = useCallback((): number[] => {
    ...
    console.log('🔍 Processing current step logic:', {
      currentStepName,
      multistepforms: multistepforms.map((form) => ({
    ...
    if (
      ...
      console.log(
        `🔍 Track status/preview page detected (${currentStepName}) - using data-based completion`
      );
    ...
    multistepforms.forEach((form, index) => {
      console.log(
        `🔍 Checking step ${index}: "${form.formtype}" === "${currentStepName}" = ${form.formtype === currentStepName}`
      );
      if (form.formtype === currentStepName) {
        currentStepIndex = index;
        console.log(
          `✅ Found current step: ${currentStepName} at index ${index}`
        );
      }
    });
    ...
    if (currentStepIndex > 0) {
      for (let i = 0; i < currentStepIndex; i++) {
        ...
        console.log(
          `🟢 Marking step ${i} (${multistepforms[i].formtype}) as completed`
        );
      }
    }

    console.log('🟢 Completed steps from current step logic:', {
      currentStepName,
      currentStepIndex,
      completedIndices,
      totalSteps: multistepforms.length,
    });
  ...
  useEffect(() => {
    ...
    if (workflowId && userId) {
      // console.log('🔄 Fetching workflow data:', { workflowId, userId });
      dispatch(fetchWorkflowData({ workflowId: workflowId, userId }));
    }
  }, [workflowId, userId]);

  // Initialize completed steps from reinitialize response
  useEffect(() => {
    if (
      reinitializeDataResponse?.data?.currentStep &&
      configuration?.formSettings?.multistepforms
      ...
      if (completedStepsFromLogin.length > 0) {
        console.log(
          '🟢 Setting completed steps from login:',
          completedStepsFromLogin
        );
        setCompletedSteps(completedStepsFromLogin);
      }
    }
  }, [
  ...
  useEffect(() => {
    ...
    if (step) {
      ...
      // If trying to access address details (step 1) without completing entity profile
      if (
        stepIndex === 1 &&
        (!formData.entityProfile ||
          Object.keys(formData.entityProfile).length === 0)
      ) {
  ...
  const updateCurrentStep = (newStep: number) => {
    // Prevent direct navigation to address details if entity profile is not completed
    if (
      newStep === 1 &&
      (!formData.entityProfile ||
        Object.keys(formData.entityProfile).length === 0)
    ) {
      // If trying to access address details directly, redirect to entity profile
      navigate(`/${STEP_URLS[0]}`, { replace: true });
      return;
    }
  ...
  const handleEditStep = (stepIndex: number) => {
    setCurrentStep(stepIndex);
    console.log(stepIndex, 'step1');
    // navigate(`/${STEP_URLS[stepIndex]}`);
    if (stepIndex === 0) {
      navigate('/modify-entity-profile');
    } else if (stepIndex == 1) {
      navigate(`/modify-address-details?step=${stepIndex}`);
    } else if (stepIndex === 2) {
      navigate('/modify-head-of-institution');
    } else if (stepIndex == 3) {
      navigate('/modify-nodal-officer');
    } else if (stepIndex === 4) {
      navigate('/modify-admin-user-details');
    }
  };
  ...
  const handleFinalSubmit = async (ackNo: string) => {
    try {
      ...
      localStorage.removeItem('modifiableFields');
...
export default TrackStatusEditContainer;
