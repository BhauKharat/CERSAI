...
const FormPreviewStep: React.FC<FormPreviewStepProps> = ({
  ...
  const isModifiableField = (fieldName: string): boolean => {
    const normalizedFieldName = (() => {
      switch (fieldName) {
        case 'correspondenceAddressLine1':
          return 'correspondenceLine1';
        case 'correspondenceAddressLine2':
          return 'correspondenceLine2';
        case 'correspondenceAddressLine3':
          return 'correspondenceLine3';
        case 'hoiMobileNumber':
          return 'hoiMobile';
        case 'hoiLandlineNumber':
          return 'hoiLandline';
        case 'noNobileNumber':
          return 'noMobileNumber';
        case 'noLandlineNumber':
          return 'noLandline';
        case 'iauLandLineNumber1':
          return 'iauLandlineNumber1';
        case 'iauLandLineNumber2':
          return 'iauLandlineNumber2';
        default:
          return fieldName;
      }
    })();
    ...
    const isModifiable = Object.values(modifiableFields).some((fields) => {
      if (Array.isArray(fields)) {
        const found = fields.includes(normalizedFieldName);
        return found;
      }
      return false;
    });
  ...
  const renderPreviewField = (field: FormField) => {
    ...
    const constitutionValue =
      (formValues['constitution'] as string) ||
    ...
    if (field.fieldName === 'proprietorName') {
      ...
      isRequired = field.validationRules?.required || field.isRequired || false;
    }
      ...
      case 'dropdown': {
        ...
        if (
          ...
          const match = field.fieldOptions.find((opt) => {
            ...
            return option?.code === value || option?.isocode === value;
  ...
  const getStepIndex = (groupName: string) => {
    ...
    if (name.includes('entity') || name.includes('profile')) {
      ...
      name.includes('admin') ||
      name.includes('user') ||
      groupName === 'reAdminone' ||
      groupName === 'reAdmintwo'
    ) {
