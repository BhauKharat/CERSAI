...
// Add props interface
interface IPAddRemoveApprovalProps {
  workflowType: string;
}

// Utility function to get label based on workflow type
const getWorkflowLabels = (workflowType: string) => {
  const labelMap: Record<string, { breadcrumb: string; header: string }> = {
    IP_WHITELISTING_ADD_NEW_IP_ADDRESS: {
      breadcrumb: 'Add New IP',
      header: 'Add New IP Address',
    },
    IP_WHITELISTING_REMOVE_IP_ADDRESS: {
      breadcrumb: 'Remove IP',
      header: 'Remove IP Address',
    },
  };

  return labelMap[workflowType] || { breadcrumb: 'IP Whitelisting', header: 'IP Whitelisting' };
};

const IPAddRemoveApproval: React.FC<IPAddRemoveApprovalProps> = ({ workflowType }) => {
  ...
  const [sortField, setSortField] = useState<keyof IPAddRemoveData | null>(null);
  const [sortDirection, setSortDirection] = useState<'asc' | 'desc'>('asc');

  // State for selected checkboxes (multiple selection)
  const [selectedIds, setSelectedIds] = useState<string[]>([]);

  // Fetch pending requests on mount and when page or workflowType changes
  useEffect(() => {
    dispatch(
      fetchPendingRequests({
  ...
  }, [dispatch, currentPage, pageSize, workflowType]);

  // Map API data to table format
  const requestData: IPAddRemoveData[] = useMemo(() => {
    ...
    try {
      ...
      await dispatch(
        approveMultipleIPWhitelistRequests({
          ...
          remarks: 'IP whitelisting approval ok',
        })
      ).unwrap();
  ...
  const handleSort = (field: keyof IPAddRemoveData) => {
    const isAsc = sortField === field && sortDirection === 'asc';
    setSortDirection(isAsc ? 'desc' : 'asc');
    setSortField(field);
  };

  // Get dynamic labels based on workflow type
  const workflowLabels = getWorkflowLabels(workflowType);

  const breadcrumbItems = [
    { label: 'My Task', href: '/re/dashboard' },
    { label: 'IP Whitelisting', href: '/re/ip-whitelisting/dashboard' },
    { label: workflowLabels.breadcrumb },
  ];
            ...
            <Table>
              ...
              <TableBody>
                {sortedData.length === 0 ? (
                  ...
                  sortedData.map((row: IPAddRemoveData, index: number) => (
                    <TableRow
                      key={row.id}
                      sx={{
                        '&:hover': {
                    ...
                    >
                      ...
                      >
                        {row.submittedOn !== '-' ? DateUtils.formatOnlyDate(row.submittedOn) : '-'}
                        <Box
                          sx={{
                            position: 'absolute',
                            right: '11px',
...
export default IPAddRemoveApproval;