...
import { fetchApplications } from '../myTask/slices/applicationSlice';
import { Application } from '../myTask/types/applicationTypes';
import { hideLoader } from '../../loader/slices/loaderSlice';
import CircleIcon from '@mui/icons-material/Circle';
import '../myTask/Dashboard.css';
...
const ALL_STATUSES = ['PENDING', 'RESUBMISSION'];

const statusMap: Record<string, string[]> = {
  All: ALL_STATUSES,
  PendingLevel1: ['RESUBMISSION'], // 🔹 Level 1
  PendingLevel2: ['PENDING'],
};

const ModifiedRegistration: React.FC = () => {
  ...
  const tableData = React.useMemo(() => {
    ...
    return data.content.map((app: Application, index: number) => {
      ...
      // Status to level mapping: RESUBMISSION -> L1, PENDING -> L2
      const status = app?.payload?.approval?.approvalStatus ?? 'PENDING';
      const pendingAtStage =
        status === 'RESUBMISSION' ? 1 : status === 'PENDING' ? 2 : 1;
  ...
  const handleViewDetails = (
    ...
    navigate(
      `/ckycrr-admin/my-task/modified-requests-view/${acknowledgementNo}/${workflowIdDetail}/${userId}`
    );
  };
...
export default ModifiedRegistration;
