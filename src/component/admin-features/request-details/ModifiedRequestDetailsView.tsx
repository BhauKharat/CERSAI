...
const ModifiedRequestDetailsView: React.FC = () => {
  ...
  const renderValue = (field: any, sectionKey?: string) => {
    ...
    if (dataPath && applicationDetailsData?.payload) {
      ...
      if (
        ...
        actualValue = formatCountryCode(actualValue, field.fieldName, field);
    ...
    if (applicationDetailsData?.payload) {
      ...
      if (value !== undefined && value !== null && value !== '') {
        ...
        // Format country code if applicable
        value = formatCountryCode(value, field.fieldName, field);
    ...
    if (
      ...
      // Format country code if applicable
      fallbackValue = formatCountryCode(fallbackValue, field.fieldName, field);
  ...
  //   fieldKey: string
  // ) => {
  //   // Ensure dependent fields toggle together when a parent (e.g., State) is toggled
  //   const dependentFieldMap: Record<string, string[]> = {
  //     registerCountry: [
  //       'registerState',
  //       'registerDistrict',
  ...
  //   Object.entries(dependentFieldMap).forEach(([parent, deps]) => {
  //     deps.forEach((dep) => {
  //       if (!parentFieldMap[dep]) parentFieldMap[dep] = parent;
  //     });
  //   });
  ...
  //     const currentSectionFields = prev[section] || [];
  //     const isSelected = currentSectionFields.includes(fieldKey);
  //     const parentField = parentFieldMap[fieldKey];
  //     const isParentSelected = parentField
  //       ? currentSectionFields.includes(parentField)
  //       : false;
  //     const parentLabel =
  //       parentField && parentField.toLowerCase().includes('country')
  //         ? 'Country'
  //         : 'State';

  //     // Prevent unchecking dependent fields while parent state remains checked
  //     if (parentField && isSelected && isParentSelected) {
  //       setSnackbar({
  //         open: true,
  //         message: `Uncheck the ${parentLabel} to remove its dependent fields`,
  //         severity: 'info',
  //       });
  //       return prev;
  //     }
  ...
  const renderSection = (
    ...
    // Get modifiable fields for this section from applicationDetailsData
    // Check both root level and payload level
    const modifiableFieldsData =
      applicationDetailsData?.modifiableFields ||
      applicationDetailsData?.payload?.modifiableFields ||
      {};
    const apiKey = getApiKeyFromGroupKey(sectionKey);

    // Try multiple case variations to match the API response
    let modifiableFieldsForSection = modifiableFieldsData[apiKey] || [];

    // If not found, try lowercase version
    if (modifiableFieldsForSection.length === 0) {
      const lowerApiKey = apiKey.toLowerCase();
      modifiableFieldsForSection = modifiableFieldsData[lowerApiKey] || [];
    }

    // If still not found, try the sectionKey directly
    if (modifiableFieldsForSection.length === 0) {
      modifiableFieldsForSection = modifiableFieldsData[sectionKey] || [];
    }

    // If still not found, try lowercase sectionKey
    if (modifiableFieldsForSection.length === 0) {
      const lowerSectionKey = sectionKey.toLowerCase();
      modifiableFieldsForSection = modifiableFieldsData[lowerSectionKey] || [];
    }

    const hasModifiableFields = modifiableFieldsForSection.length > 0;
    ...
    return (
      <StyledAccordion defaultExpanded>
        <StyledAccordionSummary
          ...
          sx={{
            backgroundColor: hasModifiableFields ? '#FFD952' : '#E6EBFF',
          }}
        >
                      ...
                      <Box
                        ...
                        title={
                          isFieldToggleable(sectionKey, fieldKey)
                            ? isSelected
                              ? 'Approved - Click to approve'
                              : 'Rejected - Click to reject'
                    ...
                    >
                      <Typography
                        ...
                        sx={{
                          ...
                          color: 'black',
                          fontWeight: '600',
                          backgroundColor: modifiableFieldsForSection.includes(
                            fieldKey
                          )
                            ? '#FFD952'
                            : undefined,
                          padding: modifiableFieldsForSection.includes(fieldKey)
                            ? '4px 8px'
                            : undefined,
                          borderRadius: modifiableFieldsForSection.includes(
                            fieldKey
                          )
                            ? '4px'
                            : undefined,
    ...
    <Box sx={requestDetailsStyles.container}>
      ...
      <AdminBreadcrumbUpdateProfile
        breadcrumbItems={[
          ...
          {
            label: 'Modified Registration',
            href: '/ckycrr-admin/my-task/modified-registration',
          },
          {
            label:
      ...
      {applicationDetailsData?.payload?.approvalWorkflow?.approvals &&
        applicationDetailsData.payload.approvalWorkflow.approvals.length >
          0 && (
          <Box
            sx={{
            ...
            <Box sx={{ flex: '1 1 300px', minWidth: '200px' }}>
              ...
              >
                {(() => {
                  ...
                  const actionByUserName =
                    latestApproval?.actionByUserName || '';
            ...
            <Box sx={{ flex: '1 1 300px', minWidth: '200px' }}>
              ...
              >
                {(() => {
                  ...
                  if (actionDateTime) {
                    const date = new Date(actionDateTime);
            ...
            <Box sx={{ flex: '1 1 300px', minWidth: '200px' }}>
              ...
              >
                {(() => {
                  ...
                  return (
                    latestApproval?.remarks || latestApproval?.reason || '-'
                  );
                })()}
              </Typography>
            </Box>
        ...
        <Box sx={requestDetailsStyles.modalBox}>
          ...
          >
            {(() => {
              ...
              if (status === 'RESUBMISSION' || !hasApprovalWorkflow) {
...
export default ModifiedRequestDetailsView;
