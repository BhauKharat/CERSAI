...
} from '../types/entityProfileApprovalTypes';
...
interface RawPendingApprovalItem {
  ...
  payload: {
    ...
    submission?: {
      submittedAt?: string;
      submittedBy?: string;
      cersaiApproverOne?: string;
      approvalWorkflow?: {
        pendingWith?: string;
        pendingAtLevel?: number;
        totalNoOfApprovals?: number;
        noOfApprovalRequired?: number;
        cersaiApproverOne?: string;
        approvals?: Array<{
          action?: string;
          actionAt?: string;
...
>(
  ...
  async (params, { rejectWithValue }) => {
    try {
      ...
      const url = ENDPOINTS.ENTITY_PROFILE_PENDING_APPROVALS;
      ...
      const pendingApprovals = contentArray.map(
        (item: RawPendingApprovalItem) => ({
          ...
          institutionName:
            item.payload?.entityDetails?.nameOfInstitution ||
            item.meta_data?.nameOfInstitution ||
            '',
          submittedOn: item.payload?.submission?.submittedAt || item.created_at,
          // Use cersaiApproverOne from approvalWorkflow for submittedBy, show "-" if not available
          submittedBy:
            item.payload?.submission?.approvalWorkflow?.cersaiApproverOne ||
            '-',
          acknowledgementNo:
          ...
          // Add noOfApprovalRequired for pending level logic
          noOfApprovalRequired:
            item.payload?.submission?.approvalWorkflow?.noOfApprovalRequired,
        })
      );
