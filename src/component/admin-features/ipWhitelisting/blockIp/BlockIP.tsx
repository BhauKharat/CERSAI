...
const BlockIP: React.FC = () => {
  ...
  const breadcrumbItems = [
    {
      ...
      href: '/ckycrr-admin/ip-whitelisting/block-ips',
    },
    {
  ...
  const getIpAddressesFromIds = useCallback(
    (ids: string[]): string[] => {
      ...
      return data.content
        .filter((item: BlockIPS) => ids.includes(item.id))
        .map((item: BlockIPS) => item.ipAddress);
    },
    [data]
  );
  ...
  const getErrorMessage = (): string => {
    ...
    return submitError || 'Failed to block IP addresses';
  };
  ...
  const tableData = useMemo(() => {
    ...
    return data.content.map((item: BlockIPS) => ({
      id: item.id,
      srNo: item.srNo,
      reNameFiCode:
