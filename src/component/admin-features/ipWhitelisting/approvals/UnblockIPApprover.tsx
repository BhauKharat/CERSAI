...
const UnblockIPApprover: React.FC = () => {
    ...
    try {
      await dispatch(
        fetchIPWhitelistingWorkflows({
          ...
          size: pageSize,
          sortBy: sortBy,
          workflowType: 'IP_WHITELISTING_UNBLOCKED_IP_ADDRESS',
          reId:
      ...
      console.error('Failed to fetch Unblock IP workflows:', err);
    } finally {
      dispatch(hideLoader());
    }
  };
  ...
  const handleApprove = async () => {
    if (selectedIPs.length > 0) {
      try {
        ...
        console.error('Failed to approve unblock IPs:', error);
      }
    }
  };
  ...
  const handleRejectSubmit = async (remark: string) => {
    try {
      ...
      console.error('Failed to reject unblock IPs:', error);
    }
  };
  ...
  const tableData = useMemo(() => {
    ...
    return data.content.map((item, index: number) => ({
      ...
      srNo: index + 1,
      reNameFiCode:
        `${item.payload?.reportingEntityDetails?.reName || ''} ${item.payload?.reportingEntityDetails?.reNamefiCode ? `(${item?.payload?.reportingEntityDetails?.reNamefiCode})` : ''}`.trim() ||
        '-',
      ipAddress:
      ...
      blockedOn: DateUtils.formatOnlyDate(item.created_at),
      blockedBy: item.meta_data?.userId || '-',
      submittedOn: item.created_at
        ? DateUtils.formatOnlyDate(item.created_at)
        : '-',
      submittedBy:
        item.payload?.initiatorDetails?.userId ||
        item.payload?.unblockRequestedBy ||
        item.payload?.userId ||
        '-',
      reason:
  ...
  return (
    <DashboardContainer>
                  ...
                  filteredData.map((row, index) => (
                    ...
                    >
                      ...
                      >
                        <Button
                          variant="text"
                          onClick={() => handleViewRemark(row.reason)}
                          sx={{
                            textTransform: 'none',
                            color: '#002CBA',
                            fontFamily: 'Gilroy, sans-serif',
      ...
      <SuccessModal
        ...
        title={
          actionType === 'approve'
            ? 'Selected IP Addresses are unblocked'
            : 'Request rejected for unblocking selected IP Addresses'
        }
        okText="Okay"
      />
...
export default UnblockIPApprover;
