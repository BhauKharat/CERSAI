...
import { fetchUserProfileFields } from '../request-details/slices/applicationPreviewSlice';
...
const UserProfileDetails: React.FC = () => {
  ...
  // Redux state for user profile approval
  const {
    workflowDetails,
    detailsLoading,
    detailsError,
    ...
    actionMessage,
    actionError,
  } = useAppSelector((state: any) => state.userProfileApproval || {});
  ...
  // Fallback: Get flat fields array if groupedFields is empty
  const fieldsArray = useSelector(
    (state: any) => state.applicationDetails?.data?.fields || []
  );
  ...
  // Get the actual user profile data from nested structure
  const profileData = useMemo(() => {
    return (
      payload?.profile_update_initiated?.updateData ||
      payload?.updateData ||
      payload ||
      {}
    );
  ...
  // Get changedFields array from payload (flat array of field names)
  const changedFields = useMemo(() => {
    return (
      payload?.profile_update_initiated?.changedFields ||
      payload?.changedFields ||
      workflowDetails?.changedFields ||
      []
    );
  }, [payload, workflowDetails]);

  // Check if a field is in the changedFields array
  ...
  useEffect(() => {
    ...
    // Fetch dynamic form fields (RE_nodal API)
    dispatch(fetchUserProfileFields());

    return () => {
      dispatch(clearUserProfileWorkflowDetails());
      dispatch(resetUserProfileActionState());
    };
  }, [dispatch, workFlowId]);
  ...
  const fetchPreviousVersion = async () => {
    // Get userId from workflowDetails
    const userId =
      workflowDetails?.userId ||
      workflowDetails?.payload?.userId ||
      payload?.profile_update_initiated?.userId ||
      payload?.userId ||
      profileData?.noEmployeeId ||
      profileData?.userId;

    // Determine user type from the workflow or payload
    const userType =
      workflowDetails?.userType ||
      workflowDetails?.payload?.userType ||
      payload?.profile_update_initiated?.userType ||
      payload?.userType ||
      'NO'; // Default to NO (Nodal Officer)
    ...
    try {
      const response = await Secured.post(
        API_ENDPOINTS.user_profile_previous_version,
        {
  ...
  const renderValue = (field: any) => {
    ...
    // First try to get value from dataPath
    if (dataPath && profileData) {
      let actualValue = getNestedValue(profileData, dataPath);
    ...
    // Try direct field name lookup in profileData (the actual user data)
    if (profileData && profileData[field.fieldName] !== undefined) {
      let value = profileData[field.fieldName];
    ...
    // Fallback: recursive search in payload
    if (payload) {
  ...
  // Check if groupedFields has actual data (must be object with keys, not empty array)
  const hasGroupedFields = useMemo(() => {
    if (!groupedFields) return false;
    // If it's an array (empty or not), don't use it - use flat fields instead
    if (Array.isArray(groupedFields)) return false;
    // Check if it's an object with actual section keys
  ...
  // Get fields from flat array (fallback when groupedFields is empty)
  const getFlatFields = () => {
  ...
  const getDisplayTitle = (groupedFieldKey: string): string => {
    ...
    const groupFieldLabelMap: Record<string, string> = {
      reNodal: 'User Details',
      renodal: 'User Details',
      nodalOfficer: 'User Details',
      nodalofficer: 'User Details',
  ...
  const isCkycFieldVerified = (fieldKey: string, fieldValue: any): boolean => {
    const ckycFields = ['noCkycNumber', 'noCkycNo'];
  ...
  const handleSuccessModalClose = () => {
    ...
    navigate('/ckycrr-admin/my-task/update-profile/user-profile');
  };
    ...
    <Box sx={styles.container}>
      ...
      <AdminBreadcrumbUpdateProfile
        breadcrumbItems={[
          ...
          {
            label: 'User Profile',
            href: '/ckycrr-admin/my-task/update-profile/user-profile',
          },
          { label: 'User Details' },
        ]}
      />
      ...
      {/* Dynamic Sections from groupedFields or flat fields */}
      <Box>
        {hasGroupedFields ? (
          ...
          // Fallback: Render flat fields in a single section
          <React.Fragment>
            {renderSection('User Details', getFlatFields(), 'reNodal')}
          </React.Fragment>
        ) : (
          // No fields available
          <StyledAccordion defaultExpanded>
      ...
      <SuccessModal
        ...
        title={
          ...
          (successModalType === 'reject'
            ...
            : 'User profile submitted for Level 2 approval')
        }
        okText="Okay"
      ...
      >
        ...
        >
          ...
          >
            {previousVersionData?.rows?.[0]?.data ? (
              <Grid container spacing={2}>
                {Object.entries(
                  previousVersionData.rows[0].data.nodalOfficer ||
                    previousVersionData.rows[0].data.headOfInstitution ||
                    previousVersionData.rows[0].data.adminUser ||
                    previousVersionData.rows[0].data ||
                    {}
                ).map(([key, value]) => {
                  ...
                  const formatLabel = (str: string) => {
                    return str
                      .replace(/^no/, '') // Remove 'no' prefix for nodal officer
                      .replace(/^hoi/, '') // Remove 'hoi' prefix for head of institution
                      .replace(/^iau/, '') // Remove 'iau' prefix for admin user
                      .replace(/([A-Z])/g, ' $1')
